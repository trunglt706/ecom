<script>
    ...
    function ViewModel() {
        ...
        self.doDel = function () {
            $.ajax({url: '{{ $uri }}/delete', type: 'post', data: {_token: '{{ csrf_token() }}', ids: JSON.stringify(self.ids())},
        ...
        self.fetch = function (filters) {
            $.ajax({url: '{{ $uri }}', type: 'post', data: { _token: '{{ csrf_token() }}', pagenum: self.pagenum, pagesize: self.pagesize, search: self.search, sort: self.sortdatafield, order: self.sortorder, filters:filters},
    ...
    $('.ckeditor').each(function(index, el) {
        @if( file_exists(config('data.PATH_MODEL').'/FileManager/') )
            <?php
                ...
                $filemanager_path = Path::url( $prefix.'filemanager?akey='.$akey);
            ?>
