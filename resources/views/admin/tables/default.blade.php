            ...
            <tr>
                ...
                @foreach($M::cols() as $key=>$val)
                    @if(in_array('srt', $val['filter']))
                        <th class="text-{{ $val['align'] }} sortdatafield" data-bind="click: sort.bind(this,'{{ $key }}')">{{ \Language::getCom(strtolower($route->extension_name).'.lbl_'.$key) }} <span style="width:14px" data-bind="attr: {'class': sortdatafield() != '{{ $key }}' ? 'fa fa-sort' : sortorder()=='asc' ? 'fa fa-sort-amount-asc' : 'fa fa-sort-amount-desc'}"></span></th>
                    @else
                        <th class="text-{{ $val['align'] }}">{{ \Language::getCom(strtolower($route->extension_name).'.lbl_'.$key) }}</th>
                    @endif
                @endforeach
                <th></th>
            </tr>
