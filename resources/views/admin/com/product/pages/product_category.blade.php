                    ...
                    <form role="form" id="edit-form">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label" for="category_name">{{ \Language::getCom('product.lbl_category_name') }} <sup class="text-danger">(*)</sup></label>
...
<script>
    ...
    function ViewModel() {
        ...
        self.add = function () {
            ...
            self.view('form');
            self.method('add');
            $('#product_custom_fields').select2('val', '');
        };
        self.edit = function (id) {
            $.post('{{ $uri }}', {_token: '{{ csrf_token() }}', id: id}, function (data) {
                ...
                self.method('update');
                self.active(id);
                $('#product_custom_fields').select2('val', JSON.parse(data.product_custom_fields));
            });
        };
        self.back = function () {
            self.view('table');
        ...
        self.doSave = function(){
            ...
            self.current().product_custom_fields = JSON.stringify($('#product_custom_fields').val());
