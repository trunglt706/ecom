            ...
            switch($field->input_type){
                case 'input':
                    if($field->unit != '') {
                        ...
                        echo '<input type="'.$field->data_type.'" '.($field->data_type=='number'?'min="0"':'').' class="form-control" data-bind="value: proInfo().field_'.$field->id.'" '.($field->not_null == 1 ? 'required':'').'>';
                    ...
                    break;
                case 'textarea':
                    echo '<textarea class="form-control" data-bind="value: proInfo().field_'.$field->id.'" '.($field->not_null == 1 ? 'required':'').'></textarea>';
                    break;
            }
        ?>
    </div>
